...
export default function ProjectDetailPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    fetchProjectBySlug(params.slug).then((p) => {
    ...
    <div className="min-h-screen py-24">
      <div className="container-max max-w-4xl">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
          ...
          <p className="text-xl text-muted-foreground mb-8 leading-relaxed">{project.tagline || project.description}</p>
          <div className="flex gap-3">
            {project.github_url && !project.is_private && (
              <a href={project.github_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border border-border hover:bg-muted">
                <Github size={18} /> View Code
              </a>
            )}
            {project.live_url && (
              <a href={project.live_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-primary text-primary-foreground">
        ...
        {project.cover_image && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass rounded-2xl overflow-hidden mb-16">
