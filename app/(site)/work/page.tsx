...
export default function WorkPage() {
  ...
  useEffect(() => {
    fetchPublishedProjects().then((data) => { setProjects(data); setLoading(false); });
  }, []);
  ...
  if (loading) {
    return <div className="min-h-screen pt-28 flex justify-center"><div className="w-8 h-8 border-2 border-primary/30 border-t-primary rounded-full animate-spin" /></div>;
  }
      ...
      <div className="container-max">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-16">
          <div className="mb-4 flex items-center gap-3">
            ...
            <span className="font-mono-custom text-xs uppercase tracking-[0.2em] text-primary">Selected work · {projects.length} projects</span>
          </div>
          ...
          <p className="text-muted-foreground md:text-lg max-w-2xl leading-relaxed">
            Projects managed from the admin panel — published work appears here automatically.
          </p>
        </motion.div>
        ...
        {categories.length > 1 && (
          <div className="mb-12 flex flex-wrap gap-2">
            {categories.map((cat) => (
              <button key={cat} onClick={() => setActiveCategory(cat)} className={`rounded-full border px-4 py-2 text-sm font-medium transition-all ${activeCategory === cat ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground'}`}>
                {cat}
              </button>
            ))}
          </div>
