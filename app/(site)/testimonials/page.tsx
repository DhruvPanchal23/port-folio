  ...
  return (
    <div className={`glass rounded-2xl ${large ? 'p-8 md:p-12 mb-12' : 'p-6'}`}>
      <div className="flex items-center gap-1 mb-4">
        {Array.from({ length: t.rating }).map((_, i) => <Star key={i} size={large ? 20 : 16} className="text-primary fill-primary" />)}
      </div>
      <p className={`text-foreground leading-relaxed mb-6 ${large ? 'text-xl md:text-2xl italic' : ''}`}>&quot;{t.content}&quot;</p>
...
export default function TestimonialsPage() {
  ...
  useEffect(() => {
    fetchPublishedTestimonials().then((data) => { setTestimonials(data); setLoading(false); });
  }, []);
